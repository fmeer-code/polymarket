...
class TokenState:
    ...
    def __init__(self, lookback_seconds: int = LOOKBACK_SECONDS):
        ...
        # Rolling history of last trade prices: deque[(ts_s, last_trade_price)]
    ...
    def _record_trade(self, ts_s: int, price: float, side: Optional[str], size: Optional[float]):
        ...
        self.trade_history.append((ts_s, price, size))
        self._trim_trades(ts_s)
        return {"ts": ts_s, "price": price, "size": size}
    ...
    def get_current_and_old_trade(self) -> Tuple[Optional[float], Optional[float]]:
        """
        Returns (last_trade_price, trade_price_1m_ago). If no trade in the last minute,
        old is set to the most recent trade (curr) per user request.
        """
        now_s = _now_s()
        ...
        async with lock:
            for label, asset_id in labels.items():
                ...
                if st.last_trade_price is not None and st.last_trade_ts_s is not None:
                    ...
                    size_str = f"{st.last_trade_size:.4f}" if st.last_trade_size is not None else "?"
                    trade_str = f" | last_trade={st.last_trade_price:.4f} ({side}, size={size_str}, age={trade_age}s)"
...
async def ws_run(asset_ids, labels: Dict[str, str]):
    ...
    def append_past_trades(now_s: int, bucket: List[Dict]):
        since = now_s - LOOKBACK_SECONDS
        for aid, label in label_by_asset.items():
            st = state.get(aid)
            if not st:
                continue
            for ts, price, size in st.get_recent_trades(since, now_s):
    ...
    def maybe_finish_captures(now_s: int):
        ...
        for cap in finished:
            ...
            with open(filename, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["timestamp", "token", "price", "size"])
                    ...
                    async for raw in ws:
                        ...
                        async with lock:
                            ...
                            if leader_aid in state:
                                ...
                                if cur is not None and old is not None and (cur - old) > 0.03:
                                    new_capture = {
                                        "started_at": now_s,
                                        "end_ts": now_s + LOOKBACK_SECONDS,
                                        "trades": [],
                                    }
                                    append_past_trades(now_s, new_capture["trades"])
...
def main():
    ...
    leader_token = "58264705683010163582113113020027018040589089722949891481908645097615099181412"
    lagger_token = "61582638568663561700911035440649627780865525056298886410686867706208834361635"
