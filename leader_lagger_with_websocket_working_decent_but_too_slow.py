...
TRIGGER_AMOUNT = 0.09      # USD price move on leader to trigger capture
...
TRADE_COOLDOWN_ON_ERROR = 120  # shorter cooldown when submission fails
...
def _resolve_pair(slug: str, pairing: str):
    tokens = get_tokens_for_market(slug) or {}
    required_keys = ("home-yes", "home-no", "away-yes", "away-no")
    missing = [key for key in required_keys if not tokens.get(key)]
    if missing:
        raise ValueError(f"Missing tokens for {missing} in slug '{slug}'. Got: {tokens}")

    pairings = {
        "home": {
            "name": f"{slug}-home_yes_vs_away_no",
            "labels": {"LEADER": tokens["home-yes"], "LAGGER": tokens["away-no"]},
        },
        "away": {
            "name": f"{slug}-away_yes_vs_home_no",
            "labels": {"LEADER": tokens["away-yes"], "LAGGER": tokens["home-no"]},
        },
    }

    if pairing not in pairings:
        raise ValueError(f"Unsupported pairing '{pairing}'. Choose from {list(pairings.keys())}.")
    return pairings[pairing]


def main():
    parser = argparse.ArgumentParser(description="Leader/lagger WS tracker for Polymarket soccer markets.")
    parser.add_argument(
        "--slug",
        default=os.getenv("SLUG", "elc-cov-ips-2025-12-29"),
        help="Market slug to trade (defaults to SLUG env or an example match).",
    )
    parser.add_argument(
        "--pair",
        choices=["home", "away"],
        default=os.getenv("PAIRING", "home"),
        help="Which direction to trade. Run the script twice (one per terminal) with both options to cover both teams.",
    )
    ...
    resolved = _resolve_pair(args.slug, args.pair)
    name = resolved["name"]
